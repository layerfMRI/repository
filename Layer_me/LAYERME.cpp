...
//fürs Histogramm
  ...
  if (!outf) {
    ...
    ofstream outfp("layer_profile.dat");
  if (!outfp) {
    cout<<"Fehler beim Oeffnen der Datei!"<<endl;
  }
  
    ofstream outfn("layer_norm.dat");
  if (!outfn) {
    cout<<"Fehler beim Oeffnen der Datei!"<<endl;
  }
...
//////////
	...
	for(int timestep = 0 ; timestep < nrep ; timestep++) {
		for(int i = 0; i< numb_layers ; i++){
			...
			outtime   << timestep  << "  " <<  vec_n[i][timestep] ; 
			outfn     << timestep  << "  " <<  vec_n[i][timestep]/gsl_stats_mean (vec_n[i],  1,nrep);
		}
