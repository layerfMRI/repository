...
void usage() { cout << "QUANT1  < Bild Anti_Bold_image > <Bild registered B1> <cutoff>" << endl;}
...
int main(int argc,char* argv[]) {
  ...
  //float T1_ = 1800. ;
  float TIeff = 0. ; 
  float chi = 0.6; // this seems to work best, even though the actuual value is morte in the rage of 0.8 -> psi is 0.8 not 0.9 may be its comming from longer pukse duration. 
  float alpha = 70.*220./270. ; 
  int SMSfac = 3. ;
  float dTI = 58.946 ; 
  float t1ratio = 0.;
...
//HAUPT SCHLEIFE 
  for(int islice=0; islice<sizeSlice; ++islice){
   ...
   //////////////////////////////////////////////////////////
   /////// here it matters if assending of desending ////////
   //////////////////////////////////////////////////////////

    //TIeff = TI + (float)(islice%slices_in_SMS_slab) * dTI ; // for ascending
    TIeff = TR - ((float)(islice%slices_in_SMS_slab) + 1. ) * dTI ; // for ascending
   ...
   for(int timestep=0; timestep<(nrep); timestep ++ ) {
	  ...
	  t1ratio = file1(timestep,islice,iy,ix) ; // t1ratio = file1(timestep,islice,iy,ix)/file2(timestep,islice,iy,ix) ;
