...
void usage() { cout << "tSNR of any time series  < Bild > <cutoff> " << endl;}
    ...
    for(int islice=0; islice<sizeSlice; ++islice){
      for(int iy=0; iy<sizePhase; ++iy){
        for(int ix=0; ix<sizeRead; ++ix){
            ...
            data1(timestep,islice,iy,ix) = file1((timestep),islice,iy,ix);   // this is VASO 
...
//cout << " bis hier3 " << endl; 
   ...
   tSNR_nulled.autowrite("tSNR_"+filename1, wopts, &prot);
   mean.autowrite("MEAN_"+filename1, wopts, &prot);
   tSNR_intens.autowrite("intens"+filename1, wopts, &prot);
